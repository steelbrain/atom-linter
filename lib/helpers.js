...
function findAsync(directory, name) {
  ...
  while (chunks.length) {
    ...
    promise = promise.then(function (filePath) {
      ...
      return names.reduce(function (promise2, name2) {
        const currentFile = Path.join(currentDir, name2);
        return promise2.then(function (filePath2) {
          if (filePath2 !== null) {
            return filePath2;
          }
...
function findCachedAsync(directory, name) {
  ...
  if (FindCache.has(cacheKey)) {
    ...
    finalValue = new Promise(function (resolve) {
    ...
    finalValue = findAsync(directory, name).then(function (filePath) {
    ...
    TMP.dir({
      ...
      Promise.all(files.map(function (file) {
        ...
        return new Promise(function (resolve2, reject2) {
          FS.writeFile(filePath, fileContents, function (error2) {
            if (error2) {

...
describe("linter helpers", function() {
  describe("::generateRange", function() {
    it("cries when invalid textEditor is passed", () =>
      expect(() => helpers.generateRange()).toThrow());
    ...
    it("returns a range pointing at file start if no or invalid line is provided", () =>
      waitsForAsync(async function() {
        ...
        expect(helpers.generateRange(textEditor)).toEqual([[0, 0], [0, 31]]);
        expect(helpers.generateRange(textEditor, -1)).toEqual([
          [0, 0],
          [0, 31]
        ]);
        // $FlowIgnore: Purposely invalid input type
        expect(helpers.generateRange(textEditor, "a")).toEqual([
          [0, 0],
          [0, 31]
        ]);
      }));
    ...
    it("ignores an invalid starting column", () =>
      waitsForAsync(async function() {
        await atom.workspace.open(somethingFile);
        const textEditor = atom.workspace.getActiveTextEditor();
        expect(helpers.generateRange(textEditor, 7, -1)).toEqual([
          [7, 0],
          [7, 43]
        ]);
        // $FlowIgnore: Purposely invalid input type
        expect(helpers.generateRange(textEditor, 7, "a")).toEqual([
          [7, 0],
          [7, 43]
        ]);
      }));
    ...
    it("cries when colStart is greater than line length", () =>
      waitsForAsync(async function() {
        await atom.workspace.open(somethingFile);
        const textEditor = atom.workspace.getActiveTextEditor();
        expect(() => helpers.generateRange(textEditor, 7, 50)).toThrow();
      }));
    ...
    it("cries when lineNumber is greater than the maximum line", () =>
      waitsForAsync(async function() {
        await atom.workspace.open(somethingFile);
        const textEditor = atom.workspace.getActiveTextEditor();
        expect(() => helpers.generateRange(textEditor, 11)).toThrow();
      }));
    ...
    it("returns a smart colEnd when starting position is provided", function() {
      waitsForAsync(async function() {
        await atom.workspace.open(mixedIndentFile);
        const textEditor = atom.workspace.getActiveTextEditor();
        expect(helpers.generateRange(textEditor, 0, 0)).toEqual([
          [0, 0],
          [0, 3]
        ]);
        expect(helpers.generateRange(textEditor, 1, 0)).toEqual([
          [1, 0],
          [1, 5]
        ]);
        expect(helpers.generateRange(textEditor, 2, 0)).toEqual([
          [2, 0],
          [2, 4]
        ]);
        expect(helpers.generateRange(textEditor, 3, 0)).toEqual([
          [3, 0],
          [3, 5]
        ]);
      });
  ...
  describe("::find", function() {
    ...
    it("works", function() {
      expect(helpers.find(__dirname, "package.json")).toBe(packageJsonPath);
      expect(helpers.findCached(__dirname, "package.json")).toBe(
        packageJsonPath
      );
    });

    it("returns null if no file is found", function() {
      expect(helpers.find("/a/path/that/does/not/exist", ".gitignore")).toBe(
        null
      );
      expect(
        helpers.findCached("/a/path/that/does/not/exist", ".gitignore")
      ).toBe(null);
    });
  });
  ...
  describe("wrapExec", function() {
    ...
    it("changes message of ENOENT errors", async function() {
      ...
      try {
        ...
        expect(error.message).toBe(
          "Failed to spawn command `some-file`. Make sure `some-file` is installed and on your PATH"
        );
    ...
    it("makes sure to kill when uniqueKey is provided", async function() {
      ...
      const firstPromise = wrapped(wait2SecondsFile, [], {
        uniqueKey: "a",
        stream: "stdout"
      });
      const secondPromise = wrapped(wait2SecondsFile, [], {
        uniqueKey: "a",
        stream: "stdout"
      });
      const thirdPromise = wrapped(wait2SecondsFile, [], {
        uniqueKey: "b",
        stream: "stdout"
      });
      const forthPromise = wrapped(wait2SecondsFile, [], {
        uniqueKey: "b",
        stream: "stdout"
      });
